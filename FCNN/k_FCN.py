...
pth = '/beegfs/desy/user/hezhiyua/2bBacked/skimmed/LLP/all_in_1/nn_format/mix/leading_jet/test/'+phase_point+'/mixing/'
...
k_inst_lst       = [pkl_df(pth, 'vbf_qcd_all_400pfck'+str(i)+'.h5') for i in range(k_fold)]
k_tb_lst         = [i.df for i in k_inst_lst]

tvt_dic          = {}
tvt_dic['test']  = pd.concat( k_tb_lst[:tst] )
tvt_dic['val']   = pd.concat( k_tb_lst[tst:val] )
tvt_dic['train'] = pd.concat( k_tb_lst[val:] )
...
for key in tvt_l:
    ...
    #tb_dict[key] = tvt_dic[key]
    #w_col        = tmp_inst.set_weights()
    #tb_dict[key]['weight'] = w_col
    print tb_dict[key][:8] 
